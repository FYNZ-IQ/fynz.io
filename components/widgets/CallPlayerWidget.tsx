...
export function CallPlayerWidget() {
  ...
  return (
    <Card ref={container} className="bg-navy-900 text-white border-white/10 rounded-[var(--r-lg)] overflow-hidden shadow-md max-w-[500px] w-full">
      <style>{`
      ...
      <div className="px-5 py-4 border-b border-white/10 flex items-center gap-3">
        ...
        <div className="flex-1">
          <b className="font-display text-[0.9rem] font-bold text-white block">Beacon Plumbing — after hours</b>
          <small className="font-mono text-[8.5px] tracking-[0.12em] text-slate-400">INBOUND CALL · 11:02 PM</small>
        </div>
        <span className={cn("font-mono text-[8.5px] tracking-[0.14em] flex items-center gap-[7px]", status.includes("LIVE") ? "text-green" : "text-slate-400")}>
          {status.includes("LIVE") && <span className="w-[7px] h-[7px] rounded-full bg-green shadow-[0_0_8px_var(--green)]"></span>}
          {status}
        </span>
      </div>
      
      <div className="flex items-center gap-[3px] h-[44px] px-5 border-b border-white/10 overflow-hidden">
      ...
      <div ref={bodyRef} className="p-5 min-h-[320px] max-h-[320px] overflow-y-auto flex flex-col gap-[9px] scroll-smooth">
        {transcript.slice(0, visibleCount).map((m) => (
          <div key={m.id} className={cn("max-w-[88%] animate-in fade-in slide-in-from-bottom-2 duration-300", 
            ...
            {m.name && <small className={cn("font-mono text-[7.5px] tracking-[0.14em] text-slate-400 block mb-1", m.type === "ai" && "text-right")}>{m.name}</small>}
            <span className={cn("block px-[13px] py-[9px] text-[0.83rem] leading-relaxed",
              m.type === "ai" ? "bg-copper-tint border border-copper/30 text-white rounded-[12px_4px_12px_12px]" :
              m.type === "sys" ? "bg-green/10 border border-green/30 text-green font-mono text-[8.5px] tracking-[0.08em] rounded-lg text-center" :
              "bg-white/[0.04] border border-white/10 text-white rounded-[4px_12px_12px_12px]"
            )}>
              {m.text}
            </span>
          </div>
