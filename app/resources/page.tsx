    ...
    <div className="flex flex-col w-full">
        ...
        <div className="wrap max-w-7xl mx-auto px-6 relative z-10 grid grid-cols-1 lg:grid-cols-[0.9fr_1.1fr] gap-12 lg:gap-14 items-center">
          ...
          >
            <div
              ...
              aria-hidden="true"
            >
              <span className="absolute top-4 left-[18px] font-mono text-[11px] tracking-[0.18em] bg-copper text-copper-ink px-3 py-[5px] rounded-full font-semibold">
                FEATURED
              </span>
              <svg
            ...
            <div className="p-6 md:px-[26px] md:py-[26px]">
              <span className="font-mono text-[11px] tracking-[0.18em] text-copper">PLAYBOOK</span>
              ...
              <div className="flex gap-3.5 mt-4 font-mono text-[11px] tracking-[0.1em] text-slate-400">
                <span>FYNZ TEAM</span>
        ...
        <div className="wrap max-w-7xl mx-auto px-6">
          <StaggerGroup className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
            {CATEGORIES.map((cat) => (
              ...
              >
                <span className="absolute top-5 right-[18px] font-mono text-[11px] tracking-[0.08em] text-slate-400">
                  {cat.count}
                </span>
      ...
      <section className="sec py-20 border-b border-line-soft">
        <div className="wrap max-w-7xl mx-auto px-6">
          <div className="max-w-3xl mb-8">
            <span className="font-mono text-[11px] tracking-[0.15em] text-copper uppercase block mb-3">Latest</span>
          ...
          <div className="flex gap-2 flex-wrap mb-7" role="group" aria-label="Filter articles">
            {FILTERS.map((f) => (
              <button
                ...
                className={cn(
                  "font-mono text-[11px] tracking-[0.12em] px-4 py-2 rounded-full border transition-all duration-200",
                  filter === f.value
            ...
            {ARTICLES.filter((a) => filter === "all" || a.cat === filter).map((art) => (
              ...
              >
                ...
                <div className="flex flex-col flex-1 px-5 pt-[18px] pb-5">
                  <span className="font-mono text-[11px] tracking-[0.16em] text-copper">{art.label}</span>
                  ...
                  <div className="flex gap-3.5 mt-3.5 font-mono text-[11px] tracking-[0.1em] text-slate-400">
                    <span>{art.date}</span>
                    <span>{art.read}</span>
                  </div>
                </div>
      ...
      <section id="docs" className="sec py-20 md:py-24 bg-card border-b border-line-soft scroll-mt-20">
        <div className="wrap max-w-7xl mx-auto px-6">
          <div className="max-w-3xl mb-14">
            <span className="font-mono text-[11px] tracking-[0.15em] text-copper uppercase block mb-3">Help yourself</span>
            <h2 className="font-display font-extrabold text-3xl md:text-4xl tracking-tight">
          ...
          <ScrollReveal className="grid grid-cols-1 lg:grid-cols-2 gap-[18px]">
            ...
            <div className="bg-navy-800 border border-white/10 rounded-[var(--r-lg)] p-8">
              ...
              {HELP_DOCS.map((doc) => (
                ...
                >
                  <span>{doc.title}</span>
                  <span className="font-mono text-[11px] text-slate-400 tracking-[0.1em] shrink-0">{doc.tag}</span>
                </Link>
              ))}
            </div>
            ...
            <div id="guides" className="bg-navy-800 border border-white/10 rounded-[var(--r-lg)] p-8 scroll-mt-24">
              ...
              {GUIDES.map((g) => (
                ...
                >
                  <span>{g.title}</span>
                  <span className="font-mono text-[11px] text-slate-400 tracking-[0.1em] shrink-0">{g.tag}</span>
                </Link>
              ))}
            </div>
          </ScrollReveal>
      ...
      <section className="relative overflow-hidden text-center py-24 md:py-28">
        ...
        <div className="wrap max-w-4xl mx-auto px-6 relative z-10">
          <span className="font-mono text-[11px] tracking-[0.15em] text-copper uppercase block mb-3">The owner&apos;s brief</span>
          ...
          <p className="mt-1.5 font-mono text-[11px] tracking-[0.1em] text-faint opacity-70">
